...
parser = argparse.ArgumentParser(description='imucal')
parser.add_argument('--source', type=str,help='csv file of source data file')
parser.add_argument('--list', type=str,help='json file of source data time list')
parser.add_argument('--fps', type=int,help='imu fps')
parser.add_argument('--out', type=str,help='json file of calibrate info  result ')
parser.add_argument('--expect_angle', type=int,help='expect angle rotated  of x,y and z direction')
args = parser.parse_args()
print(args)
data = pd.read_csv(args.source, header=0, index_col=0)
...
section_list = pd.read_json(args.list).T
...
cal = FerrarisCalibration(9.81,0-args.expect_angle)
cal_info = cal.compute(
    regions, sampling_rate_hz=args.fps, from_acc_unit="m/s^2", from_gyr_unit="rad/s", comment="My comment"
)
...
cal_info.to_json_file(args.out)
print(args.out)
...
#    cal_info, sensor_id=sys.argv[3], cal_time=datetime.now(), folder='.'
#)
#print(file_path)
...
#
#cals = find_calibration_info_for_sensor(sys.argv[3], sys.argv[3])
#cals
#
## %%
