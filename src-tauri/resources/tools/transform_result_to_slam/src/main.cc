...
{
    std::string configFileTmp = "xr3.yaml";
    std::string configFileOut;
    std::string imucal = "imu1_2020-08-12_13-21.json";
    std::string kalibr = "camchain_imucam_data.yaml";
    if (argc < 2 || argc != 5)
    {
        std::cout << "useage: ./transform_result_to_slam template.yaml imucal.json  kalibr.yaml out.yaml\n";
        exit(0);
    }
    configFileTmp = argv[1];
    imucal = argv[2];
    kalibr = argv[3];
    configFileOut = argv[4];

    if (!FileExists(configFileTmp))
    {
        std::cout << "file " << configFileTmp << " is not exist" << std::endl;
    }
    if (!FileExists(imucal))
    {
        std::cout << "file " << imucal << " is not exist" << std::endl;
    }
    if (!FileExists(kalibr))
    {
        std::cout << "file " << kalibr << " is not exist" << std::endl;
    }
    std::string trans_kalibr_format_cmd1 = "cp " + kalibr + "  " + kalibr + ".tmp";
    std::string trans_kalibr_format_cmd2 = "sed  -i '1i %YAML:1.0' " + kalibr + ".tmp";
    std::string trans_kalibr_format_cmd3 = "sed  -i 's/ - /  /'  " + kalibr + ".tmp";
    std::string trans_kalibr_format_cmd4 = "sed -i \":a;N;s/\\]\\n   \\[/,/g;$!ba\" " + kalibr + ".tmp";
    std::cout << trans_kalibr_format_cmd3 << std::endl;

    system(trans_kalibr_format_cmd1.c_str());
    system(trans_kalibr_format_cmd2.c_str());
    system(trans_kalibr_format_cmd3.c_str());
    system(trans_kalibr_format_cmd4.c_str());

    std::map<std::string, VarData> cfg;
    std::ifstream config_doc(imucal, std::ifstream::binary);
    Json::Value root;
    config_doc >> root;
